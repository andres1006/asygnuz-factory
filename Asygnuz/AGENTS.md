...
1. Read `SOUL.md` — this is who you are (Jarvis, Partner).
2. Read `USER.md` — this is who you're helping (Patroncito).
...
We build assets, not just tick checkboxes.

1.  **Centralization (The Wiki Rule):**
    *   **NEVER** dump long content solely into a Task description or comment.
    *   **ALWAYS** create a persistent document (Markdown file in `docs/` or a dedicated Page in the **Asygnuz Wiki**).
    *   The Task should only contain the **Link** to that document.

2.  **Depth & Quality (ANTI-MEDIOCRE POLICY):**
    *   **PROHIBITED:** Shallow lists (10 lines), generic bullet points, or surface-level summaries.
    *   **MANDATORY:** Deep, exhaustive analysis.
        *   **Context:** Why are we doing this? (Background).
        *   **Research:** Use `web_search` to back claims with real data.
        *   **Structure:** H1/H2 hierarchy, Comparison Tables, Detailed Pros/Cons.
        *   **Scenarios:** "Happy path" vs "Edge cases" (failure modes).
        *   **Actionable:** Concrete steps, not vague ideas.
    *   *If the output looks like a generic LinkedIn post, REJECT IT and redo.*

3.  **Traceability:**
    *   Task Status `DONE` is only valid if the Wiki Link is attached and the document meets the Depth criteria.
...
## 🧠 PROTOCOLO DE APRENDIZAJE CONTINUO (INSTINTOS)
- **Extracción de Patrones:** Al final de cada tarea, debes evaluar si hubo una corrección del usuario o un aprendizaje nuevo.
- **Registro:** Guarda estos aprendizajes en la carpeta \`instincts/\` en formato Markdown.
- **Evolución:** Antes de empezar una tarea, lee los archivos en \`instincts/\` para no repetir errores pasados.

## 🛡️ PROTOCOLO DE SEGURIDAD (AgentShield)
- Antes de cada commit a producción, el Agente Dev debe ejecutar un escaneo de seguridad.
- No se permiten credenciales en texto plano.

## 🔄 BUCLES AUTÓNOMOS
- Si una tarea es compleja, el agente puede entrar en un bucle de "Plan -> Code -> Test -> Fix" hasta que el resultado sea óptimo.

<!-- clawflows:start -->
## ClawFlows

Workflows from `/Users/andres/.openclaw/workspace/clawflows/`. When the user asks you to do something that matches an enabled workflow, read its WORKFLOW.md and follow the steps.
...
### Creating Workflows
When the user wants to create a workflow, **read `/Users/andres/.openclaw/workspace/clawflows/docs/creating-workflows.md` first and follow it.** It walks you through the interactive flow — asking questions, then creating with `clawflows create --from-json`.
...
### Workflow Locations
- **Community workflows:** `/Users/andres/.openclaw/workspace/clawflows/workflows/available/community/`
- **Custom workflows:** `/Users/andres/.openclaw/workspace/clawflows/workflows/available/custom/`
- **Enabled workflows:** `/Users/andres/.openclaw/workspace/clawflows/workflows/enabled/` (symlinks)
...
- **claudia-daily-telegram** (8am): Daily Claudia report for Telegram with strict brevity and fallback-safe output. → `/Users/andres/.openclaw/workspace/clawflows/workflows/enabled/claudia-daily-telegram//WORKFLOW.md`
- **update-clawflows** (1am): Pull the latest ClawFlows workflows and notify user of any announcements → `/Users/andres/.openclaw/workspace/clawflows/workflows/enabled/update-clawflows//WORKFLOW.md`
<!-- clawflows:end -->
