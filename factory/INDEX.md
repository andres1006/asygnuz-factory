# Factory — índice maestro

Mapa alineado con `README.md` (capas y flujo de lectura). El repositorio **wrapper** (raíz del monorepo) se describe en `../README.md`.

## Estrategia (norte)
- strategy/vision.md
- strategy/incubator-model.md

## Gobierno
- governance/operating-model.md
- governance/factory-governance.md
- governance/raci.md
- governance/quality-gates.md

## Arquitectura (Factory ↔ Template ↔ Projects)
- architecture/factory-architecture.md

## Skills (ecosistema skills.sh)
- skills/README.md

## Estándares (política transversal)
- standards/README.md
- standards/engineering-standards.md
- standards/task-specification.md
- standards/intake-documentation.md
- standards/project-changelog.md

## Decisiones (ADRs de fábrica)
- decisions/README.md
- decisions/ADR-000-template.md

## Operación
- operations/weekly-runbook.md
- operations/release-policy.md
- operations/template-evolution-process.md

## Monitoreo
- monitoring/projects-catalog.md
- monitoring/factory-dashboard.md
- monitoring/project-intake-template.md
- monitoring/project-status-template.md

## Métricas
- metrics/kpi-catalog.md
- metrics/scorecard-template.md
- metrics/quality-metrics.md

## Agentes (perfiles por rol, fuente canónica)
- agents/README.md
- agents/gate-role-map.md
- agents/handoff-flow.md
- agents/handoff-contracts.md
- agents/prompts/product.md
- agents/prompts/design.md
- agents/prompts/architecture.md
- agents/prompts/db.md
- agents/prompts/backend.md
- agents/prompts/frontend.md
- agents/prompts/qa.md
- agents/prompts/uat.md
- agents/prompts/devops.md
- agents/prompts/security.md
- agents/profiles/product.md
- agents/profiles/design.md
- agents/profiles/architecture.md
- agents/profiles/db.md
- agents/profiles/backend.md
- agents/profiles/frontend.md
- agents/profiles/qa.md
- agents/profiles/uat.md
- agents/profiles/devops.md
- agents/profiles/security.md

## Playbooks por rol
- playbooks/roles-and-handoffs.md
- playbooks/product-playbook.md
- playbooks/design-playbook.md
- playbooks/architecture-playbook.md
- playbooks/db-playbook.md
- playbooks/backend-playbook.md
- playbooks/frontend-playbook.md
- playbooks/qa-playbook.md
- playbooks/uat-playbook.md
- playbooks/devops-playbook.md
- playbooks/security-playbook.md

## Evolución del template
- changes/ (propuestas por iniciativa, p. ej. `changes/product-factory-template-v1/`)

## Documentación raíz (`factory/`)
- README.md
- INDEX.md (este archivo)
- CHANGELOG.md
